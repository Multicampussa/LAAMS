package multicampussa.laams.home.dashboard.repository;
...
public interface DashboardRepository extends JpaRepository<Notice, Long> {
...
//    ORDER BY Notice.no DESC LIMIT :count OFFSET :page
    ...
    @Query("select count(*) from Exam e where (year(e.examDate) = :year AND month(e.examDate) = :month)")
//    @Query("select n from Notice n join fetch n.manager order by n.createdAt desc")
    int findNoticeWithManagerSortByCreatedAt(@Param("year") int year, @Param("month") int month);
